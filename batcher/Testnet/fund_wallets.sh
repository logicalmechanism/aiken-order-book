...
FEE=$(${cli} transaction build \
    ...
    --tx-in ${spo_tx_in} \
    --tx-out="$(cat ${ROOT}/addresses/seller.addr)    + 10000000000" \
    --tx-out="$(cat ${ROOT}/addresses/buyer.addr)     + 10000000000" \
    --tx-out="$(cat ${ROOT}/addresses/batcher.addr)   + 10000000000" \
    --tx-out="$(cat ${ROOT}/addresses/collat.addr)    + 10000000000" \
    --tx-out="$(cat ${ROOT}/addresses/reference.addr) + 10000000000" \
    --tx-out="$(cat ${ROOT}/addresses/minter.addr)    + 10000000000" \
    ${network})
